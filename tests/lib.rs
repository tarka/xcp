...
use rand_distr::{Alphanumeric, Distribution, Pareto, Triangular};
...
fn gen_file_name(rng: &mut dyn RngCore, len: u64) -> String {
    rng.sample_iter(Alphanumeric)
        .take(len as usize)
        .collect()
}

fn gen_file(path: &Path, rng: &mut RngCore, size: usize) -> TResult {
...
fn gen_subtree(base: &Path, rng: &mut RngCore, depth: u64) -> TResult {
    create_dir_all(base)?;
...
fn gen_filetree(base: &Path, seed: u64) -> TResult {
